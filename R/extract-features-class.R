...
feature.extraction <<- function(boolTraining, stringFileTimestamp, boolLabeled) {
  ...
  for (i in 1:nrow(sensortrain)) {
    if (i > 0 & i %% wJumper == 0 & i <= nrow(sensortrain) - wJumper) {
      ...
      tmpRawXWindowLinInterp <<- predict(tmpRawLinearInterpolMod)
      ...
      tmpRawYWindowLinInterp <<- predict(tmpRawLinearInterpolMod)
      ...
      tmpRawZWindowLinInterp <<- predict(tmpRawLinearInterpolMod)
      ...
      tmpRawAWindowLinInterp <<- predict(tmpRawLinearInterpolMod)
      ...
      tmpRawBWindowLinInterp <<- predict(tmpRawLinearInterpolMod)
      ...
      tmpRawCWindowLinInterp <<- predict(tmpRawLinearInterpolMod)
      ...
      tmpRawMagnAWindowLinInterp <<- predict(tmpRawLinearInterpolMod)
      ...
      tmpRawMagnGWindowLinInterp <<- predict(tmpRawLinearInterpolMod)
