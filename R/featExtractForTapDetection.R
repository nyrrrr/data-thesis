...
sensortrain <-
  read.csv(
    paste(getwd(),"/datasets/", "17011020-sensor-dataset-training-raw.csv", sep=""), # 17011417, 17011205, 17011020, 16122802
    header = TRUE,
...
for (i in 1:nrow(sensortrain)) {
  if (i > 0 & i %% wJumper == 0 & i <= nrow(sensortrain) - wJumper) {
    ...
    tmpRawPolyMod <- lm(tmpRawAWindow ~ poly(tmpWindowCopy$Timestamp, polInterpN))
...
write.csv(
  fKeyTrain,
  "C:\\git\\data-thesis\\R\\datasets\\17011020-dataset-training-tap-detection-61.csv",
  row.names = FALSE
)
