...
for (i in seq_along(rawdata$Timestamp)) {
  rawdata$SqSumA[i] <-
    rawdata$x[i] ^ 2 + rawdata$y[i] ^ 2 + rawdata$z[i] ^ 2
  rawdata$SqSumG[i] <-
    rawdata$a[i] ^ 2 + rawdata$b[i] ^ 2 + rawdata$c[i] ^ 2
  rawdata$SqSumO[i] <-
    rawdata$alpha[i] ^ 2 + rawdata$beta[i] ^ 2 + rawdata$gamma[i] ^
    2
  
  rawdata$magnA[i] <- sqrt(rawdata$SqSumA[i])
  rawdata$magnG[i] <- sqrt(rawdata$SqSumG[i])
  rawdata$magnO[i] <- sqrt(rawdata$SqSumO[i])
}
...
for (i in 1:nrow(rawdata)) {
  if (i > 0 & i %% wJumper == 0 & i <= nrow(rawdata) - wJumper) {
    ...
    tmpRawXWindow <-
      rawdata$x[rawdata$id >= i - wJumper - 1 &
                  rawdata$id < i + wJumper]
    
    tmpRawYWindow <-
      rawdata$y[rawdata$id >= i - wJumper - 1 &
                  rawdata$id < i + wJumper]
    
    tmpRawZWindow <-
      rawdata$z[rawdata$id >= i - wJumper - 1 &
                  rawdata$id < i + wJumper]
    
    tmpRawAWindow <-
      rawdata$a[rawdata$id >= i - wJumper - 1 &
                  rawdata$id < i + wJumper]
    
    tmpRawBWindow <-
      rawdata$b[rawdata$id >= i - wJumper - 1 &
                  rawdata$id < i + wJumper]
    
    tmpRawCWindow <-
      rawdata$c[rawdata$id >= i - wJumper - 1 &
                  rawdata$id < i + wJumper]
    
    tmpRawAlphaWindow <-
      rawdata$alpha[rawdata$id >= i - wJumper - 1 &
                      rawdata$id < i + wJumper]
    
    tmpRawBetaWindow <-
      rawdata$beta[rawdata$id >= i - wJumper - 1 &
                     rawdata$id < i + wJumper]
    
    tmpRawGammaWindow <-
      rawdata$gamma[rawdata$id >= i - wJumper - 1 &
                      rawdata$id < i + wJumper]
    
    tmpRawMagnAWindow <-
      rawdata$magnA[rawdata$id >= i - wJumper - 1 &
                      rawdata$id < i + wJumper]
    
    tmpRawMagnGWindow <-
      rawdata$magnG[rawdata$id >= i - wJumper - 1 &
                      rawdata$id < i + wJumper]
    
    tmpRawMagnOWindow <-
      rawdata$magnO[rawdata$id >= i - wJumper - 1 &
                      rawdata$id < i + wJumper]
    
    tmpRawSqSumAWindow <-
      rawdata$SqSumA[rawdata$id >= i - wJumper - 1 &
                       rawdata$id < i + wJumper]
    
    tmpRawSqSumGWindow <-
      rawdata$SqSumG[rawdata$id >= i - wJumper - 1 &
                       rawdata$id < i + wJumper]
    
    tmpRawSqSumOWindow <-
      rawdata$SqSumO[rawdata$id >= i - wJumper - 1 &
                       rawdata$id < i + wJumper]
    ...
    fsd$XskewA[wIndex] <-
      3 * (fsd$XmeanA[wIndex] - fsd$XmedianA[wIndex]) / fsd$XsdA[wIndex]
    # Skewness (y) Accelerometer
    fsd$YskewA[wIndex] <-
      3 * (fsd$YmeanA[wIndex] - fsd$YmedianA[wIndex]) / fsd$YsdA[wIndex]
    # Skewness (z) Accelerometer
    fsd$ZskewA[wIndex] <-
      3 * (fsd$ZmeanA[wIndex] - fsd$ZmedianA[wIndex]) / fsd$ZsdA[wIndex]
    # Skewness (x) Gyroscope
    fsd$XskewG[wIndex] <-
      3 * (fsd$XmeanG[wIndex] - fsd$XmedianG[wIndex]) / fsd$XsdG[wIndex]
    # Skewness (y) Gyroscope
    fsd$YskewG[wIndex] <-
      3 * (fsd$YmeanG[wIndex] - fsd$YmedianG[wIndex]) / fsd$YsdG[wIndex]
    # Skewness (z) Gyroscope
    fsd$ZskewG[wIndex] <-
      3 * (fsd$ZmeanG[wIndex] - fsd$ZmedianG[wIndex]) / fsd$ZsdG[wIndex]
    # Skewness (x) Orientation
    fsd$XskewO[wIndex] <-
      3 * (fsd$XmeanO[wIndex] - fsd$XmedianO[wIndex]) / fsd$XsdO[wIndex]
    # Skewness (y) Orientation
    fsd$YskewO[wIndex] <-
      3 * (fsd$YmeanO[wIndex] - fsd$YmedianO[wIndex]) / fsd$YsdO[wIndex]
    # Skewness (z) Orientation
    fsd$ZskewO[wIndex] <-
      3 * (fsd$ZmeanO[wIndex] - fsd$ZmedianO[wIndex]) / fsd$ZsdO[wIndex]
    # RMS (x) Accelerometer
    ...
    fsd$MagnRmsA[wIndex] <- sqrt((sum((
      tmpRawMagnAWindow
    ) ^ 2)) / wSize)
    # Root Mean Square of the magnitude of the Gyroscope vector
    fsd$MagnRmsG[wIndex] <- sqrt((sum((
      tmpRawMagnGWindow
    ) ^ 2)) / wSize)
    # Root Mean Square of the magnitude of the Orientation vector
    fsd$MagnRmsO[wIndex] <- sqrt((sum((
      tmpRawMagnOWindow
    ) ^ 2)) / wSize)
    # Root Mean Square of the square sum of the Accelerometer vector
    fsd$SqSumRmsA[wIndex] <- sqrt((sum((
      tmpRawSqSumAWindow
    ) ^ 2)) / wSize)
    # Root Mean Square of the square sum of the Gyroscope vector
    fsd$SqSumRmsG[wIndex] <- sqrt((sum((
      tmpRawSqSumGWindow
    ) ^ 2)) / wSize)
    # Root Mean Square of the square sum of the Orientation vector
    fsd$SqSumRmsO[wIndex] <- sqrt((sum((
      tmpRawSqSumOWindow
    ) ^ 2)) / wSize)
    
    #peak of SqSum A
    fsd$SqSumPeakA[wIndex] <- max(tmpRawSqSumAWindow)
    #peak of SqSum G
    fsd$SqSumPeakG[wIndex] <- max(tmpRawSqSumGWindow)
    #peak of SqSum O
    fsd$SqSumPeakO[wIndex] <- max(tmpRawSqSumOWindow)
    
    #min of SqSum A
    fsd$SqSumThroughA[wIndex] <- min(tmpRawSqSumAWindow)
    #min of SqSum G
    fsd$SqSumThroughG[wIndex] <- min(tmpRawSqSumGWindow)
    #min of SqSum O
    fsd$SqSumThroughO[wIndex] <- min(tmpRawSqSumOWindow)
    
    # delta sqsum
    fsd$DeltaSqSumA[wIndex] <- fsd$SqSumPeakA[wIndex] - fsd$SqSumThroughA[wIndex]
    fsd$DeltaSqSumG[wIndex] <- fsd$SqSumPeakG[wIndex] - fsd$SqSumThroughG[wIndex]
    fsd$DeltaSqSumO[wIndex] <- fsd$SqSumPeakO[wIndex] - fsd$SqSumThroughO[wIndex]
    
    tmpTimestampWindowMaxA <-
      rawdata$Timestamp[rawdata$id >= i - wJumper - 1 &
                          rawdata$id < i + wJumper & fsd$SqSumPeakA[wIndex] == rawdata$SqSumA]
    tmpTimestampWindowMaxG <-
      rawdata$Timestamp[rawdata$id >= i - wJumper - 1 &
                          rawdata$id < i + wJumper & fsd$SqSumPeakG[wIndex] == rawdata$SqSumG]
    tmpTimestampWindowMaxO <-
      rawdata$Timestamp[rawdata$id >= i - wJumper - 1 &
                          rawdata$id < i + wJumper & fsd$SqSumPeakO[wIndex] == rawdata$SqSumO]
    tmpTimestampWindowMinA <-
      rawdata$Timestamp[rawdata$id >= i - wJumper - 1 &
                          rawdata$id < i + wJumper & fsd$SqSumThroughA[wIndex] == rawdata$SqSumA]
    tmpTimestampWindowMinG <-
      rawdata$Timestamp[rawdata$id >= i - wJumper - 1 &
                          rawdata$id < i + wJumper & fsd$SqSumThroughG[wIndex] == rawdata$SqSumG]
    tmpTimestampWindowMinO <-
      rawdata$Timestamp[rawdata$id >= i - wJumper - 1 &
                          rawdata$id < i + wJumper & fsd$SqSumThroughO[wIndex] == rawdata$SqSumO]
    
    # time gap between sqsum peak and through
    fsd$SqSumTimeGapA[wIndex] <-
      abs(tmpTimestampWindowMaxA - tmpTimestampWindowMinA)
    fsd$SqSumTimeGapG[wIndex] <-
      abs(tmpTimestampWindowMaxG - tmpTimestampWindowMinG)
    fsd$SqSumTimeGapO[wIndex] <-
    ...
    #sd of sqsum
    fsd$SsdA[wIndex] <- sd(tmpRawSqSumAWindow)
    fsd$SsdG[wIndex] <- sd(tmpRawSqSumGWindow)
    fsd$SsdO[wIndex] <- sd(tmpRawSqSumOWindow)
    
    # time of window
    fsd$TotalTime[wIndex] <-
      rawdata$Timestamp[i + wJumper - 1] - rawdata$Timestamp[i - wJumper - 1]
...
write.csv(
  fsd,
  "C:\\git\\data-thesis\\R\\datasets\\16122802-dataset-test-wsize-13.csv",
  row.names = FALSE
)